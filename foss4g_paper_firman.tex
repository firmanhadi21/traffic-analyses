...
\begin{abstract}
Advanced spatiotemporal traffic analysis has traditionally required proprietary GIS software and specialized statistical packages, creating cost barriers that limit adoption in developing countries where congestion challenges are most acute. This paper demonstrates that a fully open-source Python stack---PySAL (esda, libpysal, giddy), GeoPandas, OSMnx, and statsmodels---can deliver publication-quality spatial statistics, network analysis, and multilevel modeling for large-scale urban traffic research. We present a complete, reproducible pipeline released as an open-source package on PyPI (\texttt{pip install traffic-congestion-pipeline}) that covers the entire workflow from automated multi-provider data collection through spatial aggregation, LISA computation, Markov chain dynamics, multilevel variance decomposition, and visualization. The pipeline is applied to over 264 million traffic observations collected at 15-minute intervals across three Indonesian cities---Jakarta (14,549 road segments), Bandung (3,069 segments), and Semarang (1,076 segments)---over 11 months, representing one of the largest open-source traffic analysis efforts in a developing-country context.

The analytical workflow combines three open-source methodological components. First, PySAL's \texttt{esda.Moran\_Local} provides Local Indicators of Spatial Association (LISA) for hotspot detection, with K-nearest-neighbor spatial weights constructed via \texttt{libpysal}. Second, PySAL's \texttt{giddy} module enables classic and Spatial Markov transition analysis to quantify hotspot persistence and spatial contagion. Third, \texttt{statsmodels.mixedlm} fits multilevel mixed-effects models that rigorously partition temporal and spatial variance contributions using absolute speed (km/h) as the dependent variable---a metric choice that avoids the circularity concerns inherent in normalized jam factor indices. This multi-method integration, difficult to achieve in proprietary GUI-based tools that lack scripting flexibility for batch processing across multiple cities and time periods, showcases the composability of Python's scientific ecosystem.

The multilevel models reveal that 88--89\% of total speed variance lies between segments (ICC), reflecting fundamental differences in road design speeds. Within segments, time-of-day explains 57--67\% of speed fluctuations (temporal pseudo-$R^2$), with the evening peak (16:00--19:00) showing congestion approximately 40\% above daily averages consistently across all three cities despite their 20$\times$ population range. Adding spatial predictors---betweenness centrality (computed via OSMnx) and free-flow speed---reduces between-segment variance by 74--79\% ($\Delta R^2$), but this is almost entirely driven by free-flow speed as a proxy for road design capacity; centrality alone adds less than 1\% explanatory power beyond road type in all three cities. ANOVA validation across multiple speed metrics confirms this temporal dominance: $\eta^2$ = 8--10\% for jam factor and speed reduction, 5--6\% for absolute speed, and effectively zero for free-flow speed. All 28 pairwise period comparisons are significant (Tukey HSD, p < 0.05) for each city. Network centrality correlations with absolute current speed are near zero ($R^2$ < 0.003), while moderate centrality--jam factor correlations ($R^2$ = 0.06--0.14) are mediated through free-flow speed, confirming that the null spatial finding is genuine rather than an artifact of metric normalization.

PySAL-based spatial analysis reveals that Global Moran's I is non-significant for all cities (p > 0.35), indicating no strong overall spatial autocorrelation. Despite weak global patterns, LISA identifies meaningful local clusters in approximately 10\% of segments in Jakarta and Bandung (7.2\% in Semarang), with hotspots concentrating along known corridors such as the Sudirman-Thamrin CBD corridor in Jakarta. After Benjamini-Hochberg FDR correction, no individual segments remain significant, indicating that the spatial structure is genuine but weak---clusters exceed chance expectations by approximately 2$\times$ but do not survive stringent multiple testing correction.

LISA Markov transition analysis, implemented through \texttt{giddy.markov.Markov}, uncovers an inverse relationship between city size and hotspot persistence: Semarang shows 18.8\% probability of hotspot retention (P(HH$\rightarrow$HH)) compared to Bandung's 12.0\% and Jakarta's 6.5\%, suggesting that larger, more complex networks exhibit greater congestion volatility due to more alternative routes and greater heterogeneity in trip timing. All cities converge to similar steady-state distributions ($\sim$90\% non-significant), and no segment remained a persistent hotspot across all 8 daily periods, confirming that congestion is fundamentally temporally dynamic. Spatial Markov analysis via \texttt{giddy.markov.Spatial\_Markov} provides evidence of spatial contagion---where a segment's transition depends on its neighbors' states---in Bandung ($\chi^2$ = 8.43, p = 0.004) and Semarang ($\chi^2$ = 6.48, p = 0.011), but weaker evidence in Jakarta ($\chi^2$ = 2.54, p = 0.111), where the denser network appears to dissipate congestion spillovers more readily.

These findings, derived entirely from open-source tools, reframe urban congestion as primarily a demand synchronization problem---millions of commuters traveling simultaneously---rather than a spatial infrastructure constraint. Four independent spatial tests (betweenness centrality, POI density, road capacity, and graph-based capacity drops) converge on null results for absolute speed, strengthening this conclusion through triangulation. The practical implication is that demand management interventions (congestion pricing, flexible work schedules, public transit) may yield greater returns than capacity expansion alone in rapidly urbanizing contexts.

From a FOSS4G perspective, this work makes three contributions. First, we demonstrate that PySAL's spatial statistics modules handle real-world datasets of 264 million observations with computation times of approximately 15 minutes on consumer hardware (Mac Mini M2 Pro), establishing feasibility for operational use. Second, by releasing the complete pipeline as a PyPI package with command-line tools and Python APIs, we lower the barrier for transportation agencies and researchers in resource-constrained settings to conduct sophisticated spatiotemporal analyses without licensing costs. Third, the reproducible workflow---from data collection through final figures---serves as a template for applying FOSS4G tools to other urban analytics domains, reinforcing that open-source geospatial software is mature enough for production-grade research.

\textbf{Keywords:} open-source, reproducibility, LISA, Markov chain, spatial contagion, traffic congestion, absolute speed, multilevel model, PySAL, FOSS4G
\end{abstract}
