...
This study analyzed 264 million traffic observations across three Indonesian cities using a novel open-source Python pipeline. Multilevel variance decomposition reveals that time-of-day explains 57--67\% of within-segment speed fluctuations, while network centrality adds less than 1\% explanatory power beyond road type. ANOVA on absolute speed ($\eta^2$ = 5--6\%) and speed reduction ($\eta^2$ = 8--10\%) confirms that temporal dominance is not an artifact of jam factor normalization. Four independent spatial tests---betweenness centrality, POI density, road capacity, and capacity drops---converge on null results for absolute speed ($R^2$ < 0.003), reframing congestion as a demand synchronization problem rather than a spatial infrastructure constraint. The evening peak shows congestion approximately 40\% above daily averages. These findings support prioritizing demand management strategies alongside infrastructure expansion in rapidly urbanizing contexts.
