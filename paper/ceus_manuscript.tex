...
%% ============================================================
\begin{abstract}
Urban traffic congestion is a persistent barrier to sustainable development in rapidly urbanizing Southeast Asian cities, yet the relative importance of \emph{when} versus \emph{where} congestion occurs remains poorly quantified—particularly in developing-country contexts dominated by motorcycle traffic. This study analyzes over 264 million traffic observations collected at 15-minute intervals across three Indonesian metropolitan areas—Jakarta, Bandung, and Semarang—over 11 months using the HERE Traffic API. We combine geostatistical methods (Moran's I, LISA, Getis-Ord Gi*), network topology analysis via OSMnx, and a novel graph-based capacity drop detection framework within an open-source Python pipeline. Our central finding is that temporal factors dominate spatial factors by three orders of magnitude: time-of-day explains 15--24\% of congestion variance ($\eta^2$), while network centrality, POI density, road capacity, and bottleneck proximity each explain less than 0.01\%. Four independent spatial tests converge on null results, and H3 hexagonal aggregation at multiple scales confirms these are not modifiable areal unit artefacts. We argue that this ``temporal dominance'' reflects the fundamental nature of congestion as a demand synchronization problem—millions of commuters traveling simultaneously—rather than a spatial infrastructure constraint. Evening peak congestion exceeds daily averages by approximately 40\% across all three cities despite a 20$\times$ population range. These findings imply that demand management interventions (congestion pricing, flexible work schedules, public transit) may yield greater returns than capacity expansion alone in rapidly urbanizing contexts.
\end{abstract}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.2488}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.2390}}@{}}
\toprule\noalign{}
...
\subsection{Temporal vs.\ Spatial Effect Size Comparison}

To directly quantify the dominance of temporal factors over spatial predictors, we computed eta-squared ($\eta^2$) for temporal period from one-way ANOVA (Section 5.2.2) and compared it against $R^2$ from OLS regressions of jam factor against each spatial predictor. The results are striking: time-of-day explains 15--24\% of congestion variance, while all spatial predictors combined explain less than 0.01\%. Temporal factors dominate spatial factors by a factor of 1,000--4,000$\times$ (Table~\ref{tab:effect_size_comparison}; Figure~\ref{fig:temporal_vs_spatial}).

\textbf{Table~\ref{tab:effect_size_comparison}.} Temporal vs.\ spatial effect size comparison

\begin{longtable}[]{@{}llll@{}}
\toprule\noalign{}
City & Temporal $\eta^2$ (ANOVA) & Spatial $R^2$ (combined) & Ratio (Temporal/Spatial) \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Jakarta & 23.7\% & 0.006\% & 3,950$\times$ \\
Bandung & 19.8\% & 0.009\% & 2,200$\times$ \\
Semarang & 15.2\% & 0.015\% & 1,000$\times$ \\
...
\caption{Temporal vs.\ spatial effect size comparison across three cities}
\label{fig:temporal_vs_spatial}
\end{figure}
...
Our central finding that temporal factors dominate spatial factors by 1,000--4,000$\times$ has important theoretical and practical implications. The result reframes urban congestion fundamentally as a \textbf{demand synchronization problem} rather than a supply-side infrastructure problem. Millions of people traveling to work at roughly the same time creates congestion regardless of road network topology, capacity, or land-use density. This aligns with traffic science principles: when demand exceeds capacity, queues form (Vickrey, 1969; Arnott et al., 1993). Our empirical data from three Indonesian cities spanning a 20$\times$ population range confirms that this principle operates consistently across urban contexts.

The null results from four independent spatial tests (betweenness centrality, POI density, capacity score, and capacity drops) converge on the same conclusion. This triangulation strengthens confidence that the absence of spatial--congestion relationships is genuine rather than a methodological artefact. The H3 hexagonal aggregation (Section 5.4) confirms that null results persist at neighbourhood scales, addressing potential MAUP concerns.

The finding that high-capacity roads show marginally \textit{higher} congestion than low-capacity roads appears counterintuitive but reflects the HERE jam factor's normalization to free-flow speed. High-capacity roads (e.g., motorways) have higher free-flow speeds; when congestion occurs, the relative slowdown is proportionally larger even if absolute speeds remain higher than on lower-capacity roads. This methodological note has implications for future capacity--congestion studies: normalized congestion metrics may systematically remove capacity effects by design.
...
Several limitations warrant discussion. First, and most importantly, the HERE jam factor normalizes congestion to each segment's free-flow speed, which may remove capacity effects by construction. A motorway segment operating at 60\,km/h (against a 100\,km/h free-flow baseline) and a local road at 15\,km/h (against a 25\,km/h baseline) receive similar jam factor scores despite vastly different absolute conditions. Future work should replicate these analyses using absolute speed or volume-to-capacity ratios to disentangle whether null spatial results reflect the metric's normalization or a genuine absence of spatial structure. Second, the temporal--spatial effect size comparison uses $\eta^2$ from a categorical ANOVA (8 time periods) against $R^2$ from continuous spatial predictors; a multilevel model partitioning within-segment (temporal) and between-segment (spatial) variance would provide a more rigorous comparison. Third, we analyze three cities in a single country; generalization to cities with different urban forms (e.g., grid-planned cities, cities with extensive rail networks) requires caution. Fourth, our spatial predictors do not include every possible factor (e.g., traffic signal density, public transit routes, population density, land-use mix). Jakarta's weak but significant neighbourhood-level spatial autocorrelation detected through H3 aggregation suggests that unmeasured spatial variables may play a role. Fifth, the 11-month period, while extensive, may not capture long-term infrastructure or land-use changes.

Future research should: (1) validate findings using absolute speed as an alternative dependent variable; (2) employ multilevel models to properly partition temporal and spatial variance components; (3) apply the pipeline to cities outside Indonesia to test generalizability; (4) incorporate dynamic predictors (signal timing, transit schedules) alongside static network measures; and (5) investigate the drivers of Jakarta's neighbourhood-level spatial autocorrelation using population density and land-use data.

\section{Conclusion}

This study analyzed 264 million traffic observations across three Indonesian cities using a novel open-source Python pipeline. Our central finding is that \textbf{temporal factors dominate spatial factors by 1,000--4,000$\times$} in explaining congestion variance. Time-of-day ($\eta^2$ = 15--24\%) vastly outweighs network centrality, POI density, road capacity, and capacity drops (all $R^2$ < 0.01\%). Four independent spatial tests converge on null results, reframing congestion as a demand synchronization problem rather than a spatial infrastructure constraint. The evening peak shows congestion approximately 40\% above daily averages. These findings support prioritizing demand management strategies alongside infrastructure expansion in rapidly urbanizing contexts.
