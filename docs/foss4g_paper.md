...
**Status**: Draft complete - Ready for review and refinement
...
- ✅ Related Work (LISA, Markov, FOSS4G tools)
- ✅ Methodology (detailed technical description)
- ✅ Results with statistical tables and interpretation
- ✅ Discussion (policy implications, tool evaluation, limitations)
- ✅ Conclusion with key findings
- ✅ References (25 citations)
- ✅ Reproducible code and data availability statements
- ✅ Figure generation (all 10 required figures verified)
- ✅ Author affiliations and contributions
- ⚠️  Final proofreading and formatting for submission

**Next Steps**:
1. Review and refine technical content
2. ✅ ~~Generate high-resolution figures for submission~~ (All 10 required figures verified)
3. ✅ ~~Add author information and affiliations~~ (Complete)
4. Format according to FOSS4G 2026 submission guidelines
5. Proofread for clarity and conciseness
6. Submit to conference portal

**Figure Generation Tools**:
- Run `python verify_paper_figures.py` to check all figures
- Run `python generate_all_figures.py --verify-only` for full report
- Run `python generate_all_figures.py --high-res` to regenerate at 300 DPI
- See `FIGURE_GENERATION.md` for complete documentation
...
**Authors**:
- Firman Hadi¹ (corresponding author)
- Fauzan Abdullah²
- Yasser Wahyuddin¹
- L.M. Sabri¹

**Affiliations**:
1. Department of Geodetic Engineering, Universitas Diponegoro, Semarang, Indonesia
2. Department of Statistics, Faculty of Sciences and Mathematics, Universitas Diponegoro, Semarang, Indonesia
...
Our contribution is threefold: (1) We apply LISA Markov analysis to high-resolution traffic data across multiple cities and time periods, (2) We demonstrate the complete workflow using open-source tools (PySAL, GeoPandas, giddy), and (3) We provide actionable insights into urban congestion dynamics that can inform infrastructure planning and traffic management policies.
...
Complete code is available at: https://github.com/firmanhadi21/traffic-analyses
...
From a methodological standpoint, we showcase a complete, reproducible workflow using exclusively open-source tools. The PySAL ecosystem provides the statistical rigor, computational efficiency, and extensibility needed for operational traffic analysis, removing cost barriers that limit adoption in developing countries.
...
**Broader Impact:** As cities worldwide grapple with congestion challenges, accessible analytical methods are essential. This work contributes to the FOSS4G mission by demonstrating that advanced spatial statistics can be performed without proprietary software, democratizing traffic analysis capabilities.
...
Traffic data were collected using HERE Technologies Traffic API. The analysis was conducted using open-source software: PySAL (v2.8+), GeoPandas (v0.14+), and Python (v3.11). We thank the PySAL development team for maintaining this invaluable scientific infrastructure.
...
The following figures support the paper and are generated by the analysis scripts:

**Figure 1: Study Area and LISA Clustering (Evening Peak)**
- Files: `figures/{city}_hotspots_evening_peak.png`
- Description: Maps showing LISA cluster classifications (HH, LL, HL, LH, NS) for evening peak period (16:00-19:00)
- Cities: Jakarta, Bandung, Semarang
- Purpose: Illustrate spatial distribution of congestion hotspots and coldspots

**Figure 2: Transition Probability Matrices**
- Files: `figures/markov/{city}_transition_matrix.png`
- Description: Heatmaps showing 5×5 Markov transition matrices for LISA categories
- Cities: Jakarta, Bandung, Semarang  
- Purpose: Visualize temporal dynamics and diagonal dominance (persistence)

**Figure 3: Diagonal Dominance Comparison**
- File: `figures/markov/diagonal_dominance.png`
- Description: Bar chart comparing persistence probabilities P(i→i) across cities and states
- Purpose: Highlight systematic differences in hotspot persistence by city size

**Figure 4: Steady-State Distributions**
- File: `figures/markov/steady_state_comparison.png`
- Description: Grouped bar chart showing long-run equilibrium distribution of LISA categories
- Purpose: Demonstrate convergence to similar steady states despite different transition dynamics

**Figure 5: Spatial Contagion Test Results**
- File: `figures/markov/spatial_contagion_test.png`
- Description: Bar chart with chi-squared statistics and significance indicators
- Purpose: Visualize evidence for spatial dependence in transition probabilities

**Figure 6: Temporal Persistence Analysis**
- File: `figures/markov/persistence_analysis.png`
- Description: Multi-panel visualization showing "Ever", "Always", and "Average Duration" statistics
- Purpose: Characterize segment-level temporal stability beyond Markov transitions

---

## Appendix B: Data and Code Availability

**Code Repository**: https://github.com/firmanhadi21/traffic-analyses

**Key Scripts:**
- `compute_lisa_all_periods.py`: Computes LISA for all city-period combinations
- `compute_lisa_markov.py`: Performs Classic and Spatial Markov analysis
- `config.py`: Configuration parameters (K-neighbors, significance level, periods)

**Data Files:**
- Input: `traffic_{city}_output/{period}/aggregated_segments.gpkg`
- LISA Output: `lisa_results/{city}_{period}_lisa.gpkg`
- Markov Output: `markov_results/markov_summary.csv`
- Figures: `figures/markov/*.png`

**Reproducibility Instructions:**

```bash
# 1. Install dependencies
pip install geopandas esda libpysal giddy matplotlib seaborn

# 2. Run LISA analysis (requires traffic data GeoPackages)
python compute_lisa_all_periods.py

# 3. Run Markov analysis
python compute_lisa_markov.py

# 4. Figures will be saved to figures/markov/
# 5. Results will be in markov_results/ and lisa_results/
```

**System Requirements:**
- Python 3.11+
- 8GB RAM minimum (16GB recommended for Jakarta analysis)
- Disk space: ~2GB for results and figures

**License**: Code is available under MIT License. Figures may be reused with attribution.
...
Local Moran's I for segment $i$:

$$I_i = \frac{(x_i - \bar{x})}{m_2} \sum_{j} w_{ij}(x_j - \bar{x})$$

where $m_2 = \frac{\sum_i (x_i - \bar{x})^2}{n}$ and $w_{ij}$ are spatial weights.

Pseudo p-values computed via conditional permutation:
1. Fix $x_i$
2. Randomly permute $x_j$ values at neighboring locations
3. Recompute $I_i$ under permutation
4. Repeat 999 times
5. p-value = (# permutations with $|I| \geq |I_{obs}|$) / 1000

### Markov Transition Matrix Estimation

Given sequence $y_t \in \{0,1,2,3,4\}$ for $t = 1, \ldots, T$:

$$\hat{P}_{ij} = \frac{n_{ij}}{\sum_k n_{ik}}$$

where $n_{ij}$ = count of transitions from state $i$ at time $t$ to state $j$ at time $t+1$.

### Spatial Markov Chi-Squared Test

For each discrete lag class $\ell$, compute transition matrix $P^{(\ell)}$ conditioned on neighbors in class $\ell$.

Test statistic:

$$\chi^2 = \sum_{\ell,i,j} \frac{(O^{(\ell)}_{ij} - E^{(\ell)}_{ij})^2}{E^{(\ell)}_{ij}}$$

where $O^{(\ell)}_{ij}$ = observed transitions in lag class $\ell$, $E^{(\ell)}_{ij}$ = expected under spatial homogeneity.

Under H₀ (spatial homogeneity), $\chi^2 \sim \chi^2_{df}$ where $df$ depends on lag classes and non-zero cells.

---

## Word Count and Paper Metadata

**Estimated Word Count**: ~6,500 words (excluding code blocks, tables, and references)

**Target Conference**: FOSS4G Hiroshima 2026 - Academic Track

**Track Focus**: Demonstrating open-source geospatial tools for urban analytics

**Submission Requirements** (check conference CFP for current guidelines):
- Abstract: 250-300 words ✓
- Full paper: 6,000-8,000 words (typical academic track)  ✓
- Figures: 6-8 recommended ✓
- References: 20-30 recommended ✓
- Reproducible code: Encouraged ✓

**Author Contributions**:
- **Conceptualization**: Firman Hadi, L.M. Sabri
- **Data Collection**: Firman Hadi, Yasser Wahyuddin
- **Methodology**: Firman Hadi, Fauzan Abdullah
- **Formal Analysis**: Firman Hadi, Fauzan Abdullah
- **Software & Visualization**: Firman Hadi, Yasser Wahyuddin
- **Writing - Original Draft**: Firman Hadi
- **Writing - Review & Editing**: All authors
- **Supervision**: L.M. Sabri
...
### FOSS4G 2026 Academic Track Alignment

This paper is designed for the **Academic Track** of FOSS4G Hiroshima 2026, which emphasizes:

✅ **Novel Applications of FOSS4G Tools**: We demonstrate PySAL's giddy module for Spatial Markov analysis—underutilized in transportation research

✅ **Reproducibility**: Complete workflow with open-source tools, public code repository, and detailed methodology

✅ **Real-World Impact**: Analysis of 264 million traffic observations with policy implications for urban planning

✅ **Technical Rigor**: Permutation-based significance testing, chi-squared tests for spatial dependence, comprehensive statistical reporting

✅ **Tool Evaluation**: Critical assessment of PySAL strengths and limitations for operational traffic analysis

### Conference Submission Checklist

Before submitting to the conference portal:

- [ ] **Format**: Convert to conference template (PDF or LaTeX)
- [ ] **Figures**: Export high-resolution PNG/PDF (300 dpi minimum)
- [ ] **Length**: Verify word count fits within conference limits (typically 6,000-8,000 words)
- [ ] **Blind Review**: If required, remove author-identifying information
- [ ] **Keywords**: Ensure "FOSS4G", "PySAL", "traffic" are included
- [ ] **Abstract**: Standalone, concise summary under 300 words
- [ ] **Supplementary Materials**: Link to GitHub repository with code and data
- [ ] **Ethics/Data Statement**: Confirm data usage complies with HERE API terms
- [ ] **Proofreading**: Spell-check, grammar review, reference formatting

### Suggested Presentation Structure (if accepted)

If the paper is accepted for oral presentation (20-25 minutes typical):

**Slides Outline** (12-15 slides):
1. Title + Authors (1 slide)
2. Motivation: Traffic congestion challenges (1-2 slides)
3. Research Questions (1 slide)
4. Study Area: Three Indonesian cities (1 slide with map)
5. Methodology Overview: LISA → Markov → Spatial Markov (2-3 slides)
6. Results: Persistence patterns (1 slide)
7. Results: Spatial contagion (1 slide)
8. City Comparison: Jakarta vs Bandung vs Semarang (1 slide)
9. Discussion: Policy implications (1 slide)
10. FOSS4G Workflow: PySAL ecosystem (1 slide with code snippet)
11. Reproducibility: GitHub repo + demo (1 slide)
12. Conclusion + Future work (1 slide)

**Demo Potential**: Consider live Jupyter notebook demonstration of LISA computation for one city during presentation

---

## Keywords

Primary: LISA, Spatial Markov, Traffic Congestion, Spatiotemporal Analysis, PySAL, FOSS4G

Secondary: Urban Analytics, Hotspot Detection, Spatial Contagion, Transportation Planning, Open-Source GIS, Indonesia
...
```bibtex
@inproceedings{hadi2026foss4g_traffic,
  title={Spatiotemporal Dynamics of Traffic Congestion Hotspots: A LISA Markov Analysis of Indonesian Cities Using Open-Source Geospatial Tools},
  author={Hadi, Firman and Abdullah, Fauzan and Wahyuddin, Yasser and Sabri, L.M.},
  booktitle={Proceedings of the FOSS4G 2026 Conference},
  year={2026},
  address={Hiroshima, Japan},
  month={November},
  organization={OSGeo},
  url={https://github.com/firmanhadi21/traffic-analyses},
  note={Academic Track}
}
```
...
**Total Document Length**: ~7,500 words (main content) + 1,500 words (appendices) = ~9,000 words total

**Document Prepared**: February 21, 2026

**Contact**: Firman Hadi (firmanhadi21@lecturer.undip.ac.id)

**Repository**: https://github.com/firmanhadi21/traffic-analyses

**Affiliation**: Department of Geodetic Engineering, Universitas Diponegoro, Semarang, Indonesia
